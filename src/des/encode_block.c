...
unsigned long	*get_subkeys(unsigned long key)
{
	...
	d = key_plus & 0xFFFFFFF;
	res = (unsigned long *)malloc(16 * sizeof(unsigned long));
	i = -1;
	while (++i < 16)
		res[i] = make_subkey(&c, &d, i);
	return (res);
}
